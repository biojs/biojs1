 ...
 * 
 * @param {Object} options An object with the options for Biojs.CitationList component.
 *    
 ...
 * <ul>
 * 		<li>Biojs.CitationList.TITLE_FIRST:"TITLE_FIRST".  
 ...
 *       	</ol>
 *       </li>
 *       <li>Biojs.CitationList.AUTHORS_FIRST:"AUTHORS_FIRST". 
 *       In this case the order of the elements will be:
 *       	<ol>
 *       		<li>AUTHORS</li>
 *          	<li>TITLE</li>
 ...
 * 
 *  <ul>
 * 		<li>Biojs.CitationList.FULL_STYLE:"FULL_STYLE".  
 ...
 *       	</ol>
 *       </li>
 *       <li>Biojs.CitationList.TITLE_ONLY_STYLE:"TITLE_ONLY_STYLE". 
 ...
 * 
 *       var instance = new  Biojs.CitationList({
 *			target: 'YourOwnDivId',
 *			query:'p53',
 *	    	width: 400,
 *          height: 400,
 ...
 *	    	numberResults: 10,
 *	    	displayStyle: Biojs.CitationList.FULL_STYLE,
 *	    	elementOrder: Biojs.CitationList.TITLE_FIRST
 *	    });	
 *      
 ...
 *     
 */
Biojs.CitationList = Biojs.extend(
/** @lends Biojs.CitationList# */
{
	constructor: function(options){
		...
		if ((this.opt.elementOrder == undefined) || (this.opt.elementOrder=='')){
			this.opt.elementOrder = Biojs.CitationList.TITLE_FIRST;
		}
		
		if ((this.opt.displayStyle == undefined) || (this.opt.displayStyle=='')){
			this.opt.displayStyle = Biojs.CitationList.FULL_STYLE;
		} 
	 ...
	 * @name Biojs.CitationList-eventTypes
	 */
	eventTypes : [
      	/**
  		 * @name Biojs.CitationList#onRequestError
  		...
  		"onRequestError",
  		/**
  		 * @name Biojs.CitationList#onResultsLoaded
    ...
    */
	loadResults: function() {
		if (this.opt.width==undefined || this.opt.width ==0){
			 this.raiseEvent(Biojs.CitationList.EVT_ON_REQUEST_ERROR, {error:"width parameter mandatory"});
		}else if (this.opt.query==undefined || this.opt.query ==''){
			this.raiseEvent(Biojs.CitationList.EVT_ON_REQUEST_ERROR, {error:"query parameter mandatory"});
		}else{
				 ...
				 jQuery.ajax({
			            ...
			            success: function(resp) {
			               ...
			               if (resp.hitCount > 0){
				               ...
				               self.raiseEvent(Biojs.CitationList.EVT_ON_RESULTS_LOADED, {});
			              }else{
			            	   self.raiseEvent(Biojs.CitationList.EVT_ON_REQUEST_ERROR, {error:"Impossible to find any citation matching the query "+self.opt.query});
			              }
			            ...
			            error: function(e) {
			            	...
			            	self.raiseEvent(Biojs.CitationList.EVT_ON_REQUEST_ERROR, {error:"Generic error"});
			            }
			      });
	...
	getAbstractUrl: function(resultElement){ 
		var abstractUrl="";
		if (resultElement.source==Biojs.CitationList.PMC_SOURCE){
			abstractUrl = "http://europepmc.org/abstract/PMC/"+resultElement.id;
		}else if ((resultElement.source==Biojs.CitationList.DOI_SOURCE) || (resultElement.source==Biojs.CitationList.MED_SOURCE)){
			abstractUrl = "http://europepmc.org/abstract/med/"+resultElement.id;
		}else if (resultElement.source==Biojs.CitationList.ETH_SOURCE){
			abstractUrl = "http://europepmc.org/theses/ETH/"+resultElement.id;
		}else if (resultElement.source==Biojs.CitationList.PAT_SOURCE){
			abstractUrl = "http://europepmc.org/patents/pat/"+resultElement.id;
		}else if (resultElement.source==Biojs.CitationList.HIR_SOURCE){
			abstractUrl = "http://europepmc.org/guidelines/HIR/"+resultElement.id;
		}else if (resultElement.source==Biojs.CitationList.AGR_SOURCE){
			abstractUrl = "http://europepmc.org/abstract/AGR/"+resultElement.id;
		}else if (resultElement.source==Biojs.CitationList.CBA_SOURCE){
			abstractUrl = "http://europepmc.org/abstract/CBA/"+resultElement.id;
		}else if (resultElement.source==Biojs.CitationList.CTX_SOURCE){
    ...
    getLabelTitle: function(resultElement, fontsize, numCurrentRows, fontfamily){
		...
		if (this.opt.displayStyle == Biojs.CitationList.COMPACT_STYLE){
	...
	getSourceElement: function(resultElement){ 
		...
		}else if (resultElement.source==Biojs.CitationList.DOI_SOURCE){
			sourceString="DOI:"+resultElement.id;
		}else if (resultElement.source==Biojs.CitationList.MED_SOURCE){
	...
	buildPanel: function (resp){
		...
		for (k=0; k<numCitations; k++){
			...
			var sourceContainer;
			if (this.opt.displayStyle == Biojs.CitationList.TITLE_ONLY_STYLE){
				titleContainer = jQuery('<div class="epmc_Citation_title"></div>').appendTo(citationContainer);
			}else{
				if (this.opt.elementOrder == Biojs.CitationList.AUTHORS_FIRST){
			...
			}else if (this.opt.elementOrder == Biojs.CitationList.AUTHORS_FIRST){
				numCurrentRows=0;
				tempWidth= 0;
			...
			}else if (this.opt.elementOrder == Biojs.CitationList.TITLE_FIRST){
				numCurrentRows=0;
				tempWidth= 0;
