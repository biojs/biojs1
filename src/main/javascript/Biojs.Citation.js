 ...
 * 
 */
Biojs.Citation = Biojs.extend(
/** @lends Biojs.Citation# */
{
	...
	resetContainer : function(){
		...
		if (this.opt.displayStyle == Biojs.Citation.TITLE_ONLY_STYLE){
			this._titleContainer = jQuery('<div class="epmc_Citation_title"></div>').appendTo(this._datacontainer);
		}else{
			if (this.opt.elementOrder == Biojs.Citation.AUTHORS_FIRST){
    ...
    */
	loadCitation: function() {
		...
		if (this.opt.width==undefined || this.opt.width ==0){
			 this.raiseEvent(Biojs.Citation.EVT_ON_REQUEST_ERROR, {error:"width parameter mandatory"});
		}else if (this.opt.source==undefined || this.opt.source ==''){
			 this.raiseEvent(Biojs.Citation.EVT_ON_REQUEST_ERROR, {error:"source parameter mandatory"});
		}else if (this.opt.citation_id==undefined || this.opt.citation_id ==''){
			 this.raiseEvent(Biojs.Citation.EVT_ON_REQUEST_ERROR, {error:"citation_id parameter mandatory"});
		}else{
			...
			if (this.opt.source==Biojs.Citation.PMC_SOURCE){
				filter="PMCID:"+this.opt.citation_id;
			}else if (this.opt.source==Biojs.Citation.DOI_SOURCE){
				filter="DOI:"+this.opt.citation_id;
			}else if ((this.opt.source==Biojs.Citation.MED_SOURCE) || (this.opt.source==Biojs.Citation.PAT_SOURCE) || (this.opt.source==Biojs.Citation.AGR_SOURCE) ||
					(this.opt.source==Biojs.Citation.ETH_SOURCE) || (this.opt.source==Biojs.Citation.HIR_SOURCE) || (this.opt.source==Biojs.Citation.CBA_SOURCE) || (this.opt.source==Biojs.Citation.CTX_SOURCE)){
				 ...
				 jQuery.ajax({
			            ...
			            success: function(resp) {
			               ...
			               if (resp.hitCount > 0){
				               ...
				               self.raiseEvent(Biojs.Citation.EVT_ON_CITATION_LOADED, {});
			              }else{
			            	   self.raiseEvent(Biojs.Citation.EVT_ON_REQUEST_ERROR, {error:"Impossible to find a citation identified by id "+self.opt.citation_id+" and source "+self.opt.source});
			              }
			            ...
			            error: function(e) {
			            	...
			            	self.raiseEvent(Biojs.Citation.EVT_ON_REQUEST_ERROR, {error:"Generic error"});
			            }
			      });
			
			}else{
				this.raiseEvent(Biojs.Citation.EVT_ON_REQUEST_ERROR, {error:"Source "+this.opt.source+" not recognized"});
			}
		}
	},
	...
	buildPanel: function (resp){
		...
		if (this.opt.displayStyle == Biojs.Citation.TITLE_ONLY_STYLE){
		...
		}else if (this.opt.elementOrder == Biojs.Citation.AUTHORS_FIRST){
			...
			if (this.opt.displayStyle == Biojs.Citation.COMPACT_STYLE){
		...
		}else if (this.opt.elementOrder == Biojs.Citation.TITLE_FIRST){
			numCurrentRows=0;
			tempWidth= 0;
		...
		//ABSTRACT INFO
		if ((resp.resultList.result[0].abstractText!=null) && (this.opt.showAbstract) && (this.opt.displayStyle != Biojs.Citation.TITLE_ONLY_STYLE)){
	...
	getAbstractUrl: function(resultElement){ 
		var abstractUrl="";
		if (resultElement.source==Biojs.Citation.PMC_SOURCE){
			abstractUrl = "http://europepmc.org/abstract/PMC/"+resultElement.id;
		}else if ((resultElement.source==Biojs.Citation.DOI_SOURCE) || (resultElement.source==Biojs.Citation.MED_SOURCE)){
			abstractUrl = "http://europepmc.org/abstract/med/"+resultElement.id;
		}else if (resultElement.source==Biojs.Citation.ETH_SOURCE){
			abstractUrl = "http://europepmc.org/theses/ETH/"+resultElement.id;
		}else if (resultElement.source==Biojs.Citation.PAT_SOURCE){
			abstractUrl = "http://europepmc.org/patents/pat/"+resultElement.id;
		}else if (resultElement.source==Biojs.Citation.HIR_SOURCE){
			abstractUrl = "http://europepmc.org/guidelines/HIR/"+resultElement.id;
		}else if (resultElement.source==Biojs.Citation.AGR_SOURCE){
			abstractUrl = "http://europepmc.org/abstract/AGR/"+resultElement.id;
		}else if (resultElement.source==Biojs.Citation.CBA_SOURCE){
			abstractUrl = "http://europepmc.org/abstract/CBA/"+resultElement.id;
		}else if (resultElement.source==Biojs.Citation.CTX_SOURCE){
	...
	getSourceElement: function(resultElement){ 
		...
		}else if (resultElement.source==Biojs.Citation.DOI_SOURCE){
			sourceString="DOI:"+resultElement.id;
		}else if (resultElement.source==Biojs.Citation.MED_SOURCE){
